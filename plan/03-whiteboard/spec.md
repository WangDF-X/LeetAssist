...
- **拖动交互模型**（2026-09-22 用户修正版；09-21"绘制态完全禁拖"过界，误伤其他工具的直接拖动）：
  - **画笔态**：任何元素**不可拖**（修"画笔写字误拖已有笔画"），且起笔允许落在已有图形上（画笔不检查命中目标）；
  - **矩形/圆形/箭头/文本绘制态**：已有图形**可直接拖动**（线条类走 12px 边缘热区、矩形/椭圆为描边热区、文本整块区域）；但**新建只在空白处按下**——按在已有元素上时起笔让位给该元素的拖动（`e.target !== stage` 守卫仅这些工具保留）；
