...
**文本编辑入口（2026-09-21 用户定）**：非选择态**双击**文本元素进入编辑；选择态"点击出现边框后再点击"进入编辑（见 §5.4）；右键快速输入保持有效。
...
- **拖动交互模型**（2026-09-21 用户定稿）：
  - **绘制态**（画笔/矩形/圆形/箭头/文本工具激活时）：只绘制，**任何元素都不可拖动**（避免画笔写字时误拖已有笔画）；
  - **选择态**：① 点击元素 → 出现边框（选中）；② 边框出现后，该元素才**可拖动**、可拖**边框手柄缩放**；③ 对已选中元素**再次点击**（非拖动）→ 文本进入编辑，图形保持选中（无编辑动作）；
  - 拖动结束后紧随的 click 被忽略（`justDragged` 守卫），避免"拖完误进入编辑"。
- **文本编辑入口**：选择态"点击→边框→再点"进入编辑；绘制态**双击**文本也可进入编辑（右键快速输入始终可用，见 §5.2）。
