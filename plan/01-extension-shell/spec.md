...
### 3.7 主题与设计令牌（Design Tokens）

**模式**：浅色 / 深色 / 跟随系统三模式（`themeMode: "light" | "dark" | "system"`，默认跟随系统，持久化 `chrome.storage.local`）；跟随系统时监听 `prefers-color-scheme` 变化实时切换。切换入口为工具栏最底部单个按钮，循环顺序：浅色 → 深色 → 跟随系统，图标显示当前模式（太阳/月亮/显示器），tooltip 显示当前模式名。作用范围仅为本扩展 UI（面板、工具栏、popover、耳朵、收起按钮），不改变 LeetCode 页面本身。

**品牌色板**（源自 logo 蓝→紫渐变）：

| 角色 | 色值 | 用途 |
|---|---|---|
| accent | `#22a7f0` | 激活态、强调 |
| violet | `#8b5cf6` | 渐变高光、特殊标记（少量） |
| amber | `#fbbf24` | 点缀（通知 dot 等，极少量） |
| danger | `#e5484d` | X 按钮 hover、危险操作 |

| token | 浅色 | 深色 | 用途 |
|---|---|---|---|
| `--la-bg` | `#f7f8fa` | `#1e2128` | 面板底色 |
| `--la-surface` | `#ffffff` | `#262932` | 卡片/白板/popover 表面 |
| `--la-toolbar-bg` | `#eef1f5` | `#22252d` | 工具栏底色 |
| `--la-border` | `#d8dce3` | `#3a3e4a` | 主边框 |
| `--la-border-soft` | `#e6e9ee` | `#2e323d` | 次级分隔线 |
| `--la-text` | `#374151` | `#e5e7eb` | 主文本 |
| `--la-text-dim` | `#6b7280` | `#9aa3af` | 次文本 |
| `--la-text-faint` | `#9aa3af` | `#6b7280` | 占位/弱文本 |
| `--la-hover` | `#dde3ea` | `#31353f` | hover 背景 |
| `--la-accent` | `#22a7f0` | `#22a7f0` | 主色（双色板通用） |
| `--la-danger` | `#e5484d` | `#e5484d` | 危险色（双色板通用） |

阴影等其余 token 以 `src/panel/theme.ts` 为准。

**硬规则（全模块生效，03/04/05 及后续模块必须遵守）**：

1. 所有颜色只允许通过 `--la-*` CSS 变量引用，**禁止在组件/样式中硬编码任何颜色字面量**；
2. token 的唯一来源是 `src/panel/theme.ts`（light/dark 两块成对定义）；现有 token 不够用时在 theme.ts 中新增，不允许就地造颜色；
3. `npm run check:theme`（挂在 build 前置）扫描 `src/` 中 theme.ts 以外的颜色字面量，发现即构建失败；
4. 阴影、圆角、z-index 等非颜色值不在此限制内，但阴影颜色也必须走 token。
...
- [ ] 工具栏最底部主题按钮可循环切换 浅色/深色/跟随系统；跟随系统时随系统主题实时切换；选择持久化；面板全部 UI 元素（面板/工具栏/popover/耳朵/收起按钮）在两模式下颜色正确。
